 ...
 * @date 2022/5/6
 */
export function useVisibleControl<T extends Record<string, boolean>>( keys: T, options: HooksOption = {}): [T, SwitchControlType<T>] {
  ...
  return [
    ...
    useMemo(() => {
      ...
      const result = {
        closeAll() {
          setState(oldValue => Object.keys(oldValue).reduce((prev, curr) => {
            prev[curr] = false;
            return prev
          }, {}) as T)
        }
